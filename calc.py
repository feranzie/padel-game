...
def click_event(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # Draw a red rectangle as background for annotation
        text = f'({x_meter}, {y_meter})'
        (text_width, text_height), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
        cv2.rectangle(frame, (x, y - text_height - 10), (x + text_width, y), (0, 0, 255), -1)

        # Add annotation text in black with thicker text
        cv2.putText(frame, text, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
        print(f"x meter is {x_meter}, y meter is {y_meter}")
